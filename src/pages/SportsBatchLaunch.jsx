  ...
  return (
    <div className="min-h-screen bg-black text-white px-6 py-10">
      <div className="max-w-6xl mx-auto">
        ...
        <div className="text-center text-white/45 text-sm mt-2 mb-4">
          <span className="mx-2 text-[10px] text-[#E8DCC0]/70">•</span>
          {cards.length} card{cards.length === 1 ? "" : "s"} are ready for listing
          <span className="mx-2 text-[10px] text-[#E8DCC0]/70">•</span>
        </div>
        <div className="mt-4 mb-6">
          <div className="h-[2px] w-full rounded-full bg-gradient-to-r from-transparent via-[#E8DCC0]/70 to-transparent" />
        </div>
        {cards.length === 0 ? (
          ...
          <div className="launch-page">
            <div className="lux-card border border-white/10 p-3 mb-6">
            ...
            <div className="text-center text-white/60 text-sm mb-8">
              ...
              <span className="mx-2 text-[10px] text-[#E8DCC0]/70">•</span>
              {saveCorners ? "Corners available" : "Corners off"}
              <span className="mx-2 text-[10px] text-[#E8DCC0]/70">•</span>
            </div>

            <div className="mb-8">
              <div className="h-[3px] w-full rounded-full bg-gradient-to-r from-[#E8DCC0]/20 via-[#E8DCC0] to-[#E8DCC0]/20 animate-pulse" />
            </div>
                ...
                return (
                  <>
                    <div key={card.id} className="space-y-3">
                      <div className="lux-card border border-white/10 p-3 flex flex-wrap items-start gap-3">
                      ...
                      <div className="flex-1 min-w-0 space-y-1">
                        <div className="text-[10px] uppercase tracking-[0.25em] text-[#E8DCC0]/80">
                          Card {index + 1}
                        </div>
                        ...
                        <div className="mt-2 grid gap-2">
                          {activePlatforms.map((platformId) => {
                            const label =
                              PLATFORM_OPTIONS.find((option) => option.id === platformId)?.label ||
                              platformId;
                            const content = buildMarketplaceContent(platformId, identity);
                            ...
                            const openUrl =
                              platformId === "whatnot" ? "https://www.whatnot.com/" : exportLinks?.[platformId] || "";
                            const isDone = completedActions?.[card.id]?.[platformId] === true;
                            return (
                              <div key={card.id + "-" + platformId}>
                                <div className="hidden sm:block border border-white/10 rounded-md p-2">
                                  ...
                                  <div className="mt-1 text-xs text-white/80 truncate">{content.title || "—"}</div>
                                  {platformId === "whatnot" && content.note ? (
                                    <div className="text-[10px] text-white/50 mt-1 truncate">{content.note}</div>
                                  ) : null}
                                  {content.description ? (
                                    <div className="text-[10px] text-white/50 mt-1 truncate">{content.description}</div>
                                  ) : null}
                                ...
                                <details className="sm:hidden border border-white/10 rounded-md p-2">
                                  <summary className="list-none cursor-pointer">
                                    <div className="flex items-center justify-between gap-2">
                                      ...
                                      <div className="text-[10px] uppercase tracking-[0.2em] text-white/40">
                                        {isDone ? "Copied ✓" : "Details ▸"}
                                      </div>
                                    </div>
                                  </summary>
                                  <div className="mt-2">
                                    {platformId === "whatnot" && content.note ? (
                                      <div className="text-[10px] text-white/50 truncate">{content.note}</div>
                                    ) : null}
                                    {content.description ? (
                                      <div className="text-[10px] text-white/50 mt-1 truncate">{content.description}</div>
                                    ) : null}
